...
import { deletePeriod, listPeriods, periodErrorMessage } from '../../../services/period';
import type { PeriodListResponse, PeriodRecord } from '../../../types/period';
...
export default function PeriodListPage() {
  ...
  const loadPeriods = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const result: PeriodListResponse = await listPeriods({ page, per_page: PER_PAGE, search });
      ...
      setPeriods([]);
      setTotal(0);
      setError(periodErrorMessage(requestError, 'Unable to load periods.'));
    } finally {
      setLoading(false);
    }
  }, [page, search]);
  ...
  async function handleDelete(item: PeriodRecord) {
    if (!window.confirm(`Delete period ${item.period_name ?? 'this entry'}?`)) return;
    ...
    try {
      ...
      if (periods.length === 1 && page > 1) setPage((current) => current - 1);
      else await loadPeriods();
    } catch (requestError: unknown) {
      setError(periodErrorMessage(requestError, 'Unable to delete the period.'));
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return <ProtectedRoute><Layout><div className="space-y-6">
    <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Schedule management</p><h1 className="mt-2 text-3xl font-semibold text-slate-900">Periods</h1><p className="mt-2 max-w-2xl text-sm text-slate-600">Manage the named periods assigned to the school timetables.</p></div><button type="button" onClick={() => router.push('/periods/new')} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700">Add period</button></div>
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><form onSubmit={submitSearch} className="flex flex-col gap-3 md:flex-row"><label htmlFor="period-search" className="sr-only">Search periods</label><input id="period-search" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search by period name" className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white" /><button type="submit" className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100">Search</button>{search && <button type="button" onClick={() => { setSearch(''); setSearchInput(''); setPage(1); }} className="rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm font-medium text-red-700 hover:bg-red-100">Clear</button>}</form></div>
    {error && <Alert message={error} onRetry={() => void loadPeriods()} />}
    {success && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">{success}</div>}
    {timetableState.error && <Alert message={`Timetable labels unavailable: ${timetableState.error}`} onRetry={() => void timetableState.reload()} subtle />}
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="min-w-[820px] w-full text-left text-sm text-slate-700"><thead className="bg-slate-50 text-xs uppercase tracking-[0.14em] text-slate-500"><tr><th className="px-4 py-3 font-semibold">Name</th><th className="px-4 py-3 font-semibold">Timetable</th><th className="px-4 py-3 font-semibold">Status</th><th className="px-4 py-3 text-right font-semibold">Actions</th></tr></thead><tbody>{loading ? <tr><td colSpan={4} className="px-4 py-12 text-center text-slate-500">Loading periods…</td></tr> : periods.length === 0 ? <tr><td colSpan={4} className="px-4 py-12 text-center text-slate-500">{search ? 'No periods match your search.' : 'No periods have been created yet.'}</td></tr> : periods.map((item) => <tr key={item.id} className="border-t border-slate-200 hover:bg-slate-50"><td className="px-4 py-3 font-medium"><button type="button" onClick={() => router.push(`/periods/${item.id}`)} className="text-left text-blue-600 hover:text-blue-700">{item.period_name ?? 'Unnamed period'}</button></td><td className="px-4 py-3">{timetableLabel(timetableState.timetables?.find((timetable) => timetable.id === item.timetable_id)?.timetable_name, item.timetable_id)}</td><td className="px-4 py-3"><StatusBadge status={item.status} /></td><td className="px-4 py-3"><div className="flex justify-end gap-2"><button type="button" onClick={() => router.push(`/periods/${item.id}`)} className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100">View</button><button type="button" onClick={() => router.push(`/periods/${item.id}/edit`)} className="rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100">Edit</button><button type="button" disabled={deletingId === item.id} onClick={() => void handleDelete(item)} className="rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 text-xs font-medium text-red-700 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50">{deletingId === item.id ? 'Deleting…' : 'Delete'}</button></div></td></tr>)}</tbody></table></div>{!loading && total > 0 && <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between"><span>Page {page} of {totalPages} · {total} total</span><div className="flex gap-2"><button type="button" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page <= 1} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-medium text-slate-700 disabled:cursor-not-allowed disabled:opacity-40">Previous</button><button type="button" onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page >= totalPages} className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-medium text-slate-700 disabled:cursor-not-allowed disabled:opacity-40">Next</button></div></div>}</div>
